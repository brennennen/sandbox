...
static void fdt_parse_header(fdt_header_t* header, uint8_t* bytes) {
    uint32_t index = 0;
    header->magic = read_uint32_big_endian(bytes, &index);
}

static result_t fdt_parse_body(flattened_device_tree_t* fdt, uint8_t* bytes) {
}

result_t fdt_parse_blob(flattened_device_tree_t* fdt, uint8_t* bytes) {
    // fdt_read_header(&fdt->header, bytes);
}

result_t fdt_parse_blob_file(flattened_device_tree_t* fdt, char* file_path) {
    // TODO: read file
