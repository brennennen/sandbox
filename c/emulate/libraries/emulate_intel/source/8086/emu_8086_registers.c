...
ip: %d\n\
",
        emulator->registers.ax, emulator->registers.bx, emulator->registers.cx,
        emulator->registers.dx, emulator->registers.si, emulator->registers.di,
        emulator->registers.bp, emulator->registers.sp, emulator->registers.cs,
        emulator->registers.ds, emulator->registers.es, emulator->registers.ss,
...
void emu_8086_print_registers_condensed(emulator_8086_t* emulator) {
    ...
    if (emulator->registers.ax) {
        printf("ax: %d, ", emulator->registers.ax);
    }
    if (emulator->registers.bx) {
        printf("bx: %d, ", emulator->registers.bx);
    }
    if (emulator->registers.cx) {
        printf("cx: %d, ", emulator->registers.cx);
    }
    if (emulator->registers.dx) {
        printf("dx: %d, ", emulator->registers.dx);
    }
    if (emulator->registers.si) {
        printf("si: %d, ", emulator->registers.si);
    }
    if (emulator->registers.di) {
        printf("di: %d, ", emulator->registers.di);
    }
    if (emulator->registers.bp) {
        printf("bp: %d, ", emulator->registers.bp);
    }
    if (emulator->registers.sp) {
        printf("sp: %d, ", emulator->registers.sp);
    }
    if (emulator->registers.cs) {
        printf("cs: %d, ", emulator->registers.cs);
    }
    if (emulator->registers.ds) {
        printf("ds: %d, ", emulator->registers.ds);
    }
    if (emulator->registers.es) {
        printf("es: %d, ", emulator->registers.es);
    }
    if (emulator->registers.ss) {
        printf("ss: %d, ", emulator->registers.ss);
    }
    printf("\n");
}
...
void emu_8086_print_flags_condensed(uint16_t flags) {
    printf("flags: ");
    if (emu_reg_get_flag(flags, FLAG_OF_MASK)) {
        printf("o");
    }
    if (emu_reg_get_flag(flags, FLAG_DF_MASK)) {
        printf("d");
    }
    if (emu_reg_get_flag(flags, FLAG_IF_MASK)) {
        printf("i");
    }
    if (emu_reg_get_flag(flags, FLAG_TF_MASK)) {
        printf("t");
    }
    if (emu_reg_get_flag(flags, FLAG_SF_MASK)) {
        printf("s");
    }
    if (emu_reg_get_flag(flags, FLAG_ZF_MASK)) {
        printf("z");
    }
    if (emu_reg_get_flag(flags, FLAG_AF_MASK)) {
        printf("a");
    }
    if (emu_reg_get_flag(flags, FLAG_PF_MASK)) {
        printf("p");
    }
    if (emu_reg_get_flag(flags, FLAG_CF_MASK)) {
        printf("c");
    }
    printf("\n");
}
...
void emu_reg_update_overflow_flag_8bit(
    ...
    if (left_negative == right_negative && left_negative != result_negative) {
