...
emu_result_t emu_8086_init(emulator_8086_t* emulator) {
    emulator->stack_size =
        STACK_SIZE;  // using a size here in case i want to make this dynamic/resizable later.
...
emu_result_t emu_memory_get_uint16(
...
result_t emu_8086_emulate_file(emulator_8086_t* emulator, char* input_path) {
    ...
    LOGMEM(
        emulator->memory, emulator->registers.ip, sizeof(emulator->memory),
