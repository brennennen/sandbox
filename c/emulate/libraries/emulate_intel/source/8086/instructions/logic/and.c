 ...
 * `and [1000], bl`
 */
emu_result_t emu_and__direct_access(emulator_8086_t* emulator, direction_t direction, wide_t wide, uint8_t reg, uint8_t rm, uint16_t displacement) {
 ...
 * `and cx, bx`
 */
emu_result_t emu_and__register(emulator_8086_t* emulator, direction_t direction, wide_t wide, uint8_t reg, uint8_t rm, mod_t mod, uint16_t displacement) {
...
emu_result_t emu_and(emulator_8086_t* emulator, uint8_t byte1) {
    ...
    LOGDIW(write__common_register_or_memory_with_register_or_memory, direction, wide, mod, reg, rm, displacement, "and", 3);
...
static emu_result_t emu_add_immediate__direct_access(emulator_8086_t* emulator, wide_t wide, uint16_t displacement, uint16_t immediate) {
    if (wide == WIDE_BYTE) {
        uint8_t source_data = 0;
        int res = emu_memory_get_byte(emulator, displacement, &source_data);
        if (res != ER_SUCCESS) {
...
static emu_result_t emu_add_immediate__effective_address(emulator_8086_t* emulator, wide_t wide, uint8_t rm, mod_t mod, uint16_t displacement, uint16_t immediate) {
...
static emu_result_t emu_add_immediate__register(emulator_8086_t* emulator, wide_t wide, uint8_t rm, uint16_t immediate) {
    if (wide == WIDE_BYTE) {
...
emu_result_t emu_and_immediate(emulator_8086_t* emulator, uint8_t byte1) {
    ...
    LOGDIW(write__common_immediate_to_register_or_memory, 0, wide, mod, rm, displacement, immediate, "and", 3);
    ...
    switch(mod) {
        ...
        case MOD_MEMORY_8BIT_DISPLACEMENT:
        case MOD_MEMORY_16BIT_DISPLACEMENT: {
            return emu_add_immediate__effective_address(emulator, wide, rm, mod, displacement, immediate);
        }
        case MOD_REGISTER: {
...
void write_and_immediate_to_ax(wide_t wide, uint16_t immediate, char* buffer, int* index, int buffer_size) {
    char* reg_string = "ax";
    if (wide == WIDE_BYTE) {
        reg_string = "al";
    } else {
    ...
    int written = snprintf(buffer + *index,  buffer_size - *index, "and %s, %d",
        reg_string,
        immediate);
    if (written < 0) {
