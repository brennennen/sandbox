...
static emu_result_t write_not__effective_address() {
    printf("TODO decode_not__effective_address not implemented.\n");
    return ER_FAILURE;
}
...
static emu_result_t write_not(
    ...
    switch (mod) {
        ...
        case MOD_MEMORY_16BIT_DISPLACEMENT: {
            return write_not__effective_address();
        }
        case MOD_REGISTER: {
 ...
 * `not bx`
 */
static emu_result_t emu_not__register(emulator_8086_t* emulator, wide_t wide, uint8_t rm) {
    if (wide == WIDE_BYTE) {
