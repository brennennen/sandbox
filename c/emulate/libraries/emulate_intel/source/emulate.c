...
result_t emu_disassemble_file(
    ...
    switch (emulator->arch) {
        case ARCH_I8086: {
            return (emu_8086_disassemble_file(
                &emulator->emulator_8086, input_path, out_buffer, out_buffer_size
            ));
        }
        default: {
            LOGD("ARCH not implemented");
...
result_t emu_disassemble_chunk(
    ...
    switch (emulator->arch) {
        case ARCH_I8086: {
            return (emu_8086_disassemble_chunk(
                &emulator->emulator_8086, in_buffer, in_buffer_size, out_buffer, out_buffer_size
            ));
        }
        default: {
            LOGD("ARCH not implemented");
