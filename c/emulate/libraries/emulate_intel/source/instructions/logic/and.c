 ...
 * `and [1000], bl`
 */
emu_result_t emu_and__direct_access(emulator_t* emulator, direction_t direction, wide_t wide, uint8_t reg, uint8_t rm, uint16_t displacement) {
 ...
 * `and cl, [bx + 1000]`
 */
emu_result_t emu_and__effective_address(emulator_t* emulator) {
 ...
 * `and cx, bx`
 */
emu_result_t emu_and__register(emulator_t* emulator, direction_t direction, wide_t wide, uint8_t reg, uint8_t rm, mod_t mod, uint16_t displacement) {
...
static emu_result_t emu_add_immediate__direct_access(emulator_t* emulator, wide_t wide, uint16_t displacement, uint16_t immediate) {
    if (wide == WIDE_BYTE) {
        uint8_t source_data = 0;
        int res = emu_memory_get_byte(emulator, displacement, &source_data);
        if (res != ER_SUCCESS) {
...
static emu_result_t emu_add_immediate__effective_address(emulator_t* emulator, wide_t wide, uint8_t rm, mod_t mod, uint16_t displacement, uint16_t immediate) {
