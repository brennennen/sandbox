...
static emulator_i386_t g_emulator;

void emu_i386_decode_and_default_setup(void) {
    memset(&g_emulator, 0, sizeof(emulator_i386_t));
    emu_i386_init(&g_emulator);
...
Test(decode_i386__I_AND__tests, and__register__m32, .init = emu_i386_decode_and_default_setup) {
    ...
    char output[32] = {0x00};
    cr_assert(
        SUCCESS == emu_i386_decode_chunk(&g_emulator, input, sizeof(input), output, sizeof(output))
    );
