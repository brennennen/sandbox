 ...
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_unconditional_jumps) The offset
 * is ~21 bits, spread out in a way to make hardware easier to process them (it's also sign
 * extended). See the spec for the exact bitmapping. Only even addresses are indexable, so the least
 * significant bit is left out.
 */
static inline void rv64_decode_j_type(uint32_t raw_instruction, int32_t* offset, uint8_t* rd) {
 ...
 * @see RISC-V Unprivileged ISA Manual, Section 2.5.2 Conditional Branches
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_conditional_branches) Similar
 * to J-Type, the offset is ~13 bits, spread out in a way to make hardware easier to process them
 * (also sign extended). See spec for exact bitmapping. Only even addresses are indexable, so the
 * least significant bit is left out.
 */
 ...
 * @see RISC-V Unprivileged ISA Manual, Section 2.6. Load and Store Instructions
...
static inline void rv64v_decode_vsetvli_vtypei(
    ...
    *vlmul = vtypei & 0b111;
    printf(
        "vtypei: %d, vill: %d, vma: %d, vta: %d, vsew: %d, vlmul: %d\n", vtypei, *vill, *vma, *vta,
        *vsew, *vlmul
    );
}
