...
/**
 *
 * @see 3.1.1 Machine ISA (`misa`) Register
 * https://riscv.github.io/riscv-isa-manual/snapshot/privileged/#misa
 */
...
uint64_t rv64_get_csr_value(rv64_csrs_t* csrs, int32_t address) {
    switch (address) {
        ...
        case RV64_CSR_MVENDORID:
            return (csrs->mvendorid);
        case RV64_CSR_MARCHID:
            return (csrs->marchid);
        case RV64_CSR_MIMPID:
            return (csrs->mimpid);
        case RV64_CSR_MHARID:
            return (csrs->mhartid);
        case RV64_CSR_MCONFIGPTR:
            return (csrs->mconfigptr);
        // Machine Trap Setup
        ...
        case 0x305:
            return (csrs->mtvec);
        case 0x306:
            return (csrs->mcounteren);
        // Machine Counter Configuration
        case 0x321:
            return (csrs->mcyclecfg);
        case 0x322:
            return (csrs->minstretcfg);
        // Machine Trap Handling
        case 0x340:
            return (csrs->mscratch);
        case 0x341:
            return (csrs->mepc);
        case 0x342:
            return (csrs->mcause);
        ...
        // todo: pmpcfg 4 - 12
        case 0x3ae:
            return (csrs->pmpcfg14);
        case 0x3b1:
            return (csrs->pmpaddr0);
        case 0x3b2:
            return (csrs->pmpaddr1);
        // todo: pmpaddr 2 - 62
        case 0x3ef:
            return (csrs->pmpaddr63);
        // Machine State Enable Registers
        case 0x30c:
            return (csrs->mstateen0);
        case 0x30d:
            return (csrs->mstateen1);
        case 0x30e:
            return (csrs->mstateen2);
        case 0x30f:
            return (csrs->mstateen3);
        // Machine Non-Maskable Interrupt Handling
        case 0x740:
            return (csrs->mnscratch);
        case 0x741:
            return (csrs->mnepc);
        case 0x742:
            return (csrs->mncause);
        case 0x744:
            return (csrs->mnstatus);
        // Machine Counter/Timers
        case 0xb00:
            return (csrs->mcycle);
        case 0xb02:
            return (csrs->minstret);
        case 0xb03:
            return (csrs->mhpmcounter3);
        // todo: mhpmevent 4 - 30
        case 0xb1f:
            return (csrs->mhpmcounter31);
        // Machine Counter Setup
        case 0x320:
            return (csrs->mcountinhibit);
        case 0x323:
            return (csrs->mhpmevent3);
        // todo: mhpmevent 4 - 30
        case 0x33f:
            return (csrs->mhpmevent31);
        ...
        case 0x7b1:
            return (csrs->dpc);
        case 0x7b2:
            return (csrs->dscratch0);
        case 0x7b3:
            return (csrs->dscratch1);
        default: {
            // todo: log error
            return (0);
        }
...
uint8_t rv64_encode_vmul(rv64v_vlmul_t vlmul) {
    switch (vlmul) {
        ...
        case RV64_VLMUL_8:
            return 0b011;
        case RV64_VLMUL_HALF:
            return 0b111;
        case RV64_VLMUL_QUARTER:
            return 0b110;
        case RV64_VLMUL_EIGHTH:
            return 0b101;
...
rv64v_vlmul_t rv64_decode_vmul(uint8_t raw_vlmul) {
    switch (raw_vlmul) {
        ...
        case 0b011:
            return RV64_VLMUL_8;
        case 0b111:
            return RV64_VLMUL_HALF;
        case 0b110:
            return RV64_VLMUL_QUARTER;
        case 0b101:
            return RV64_VLMUL_EIGHTH;
        default: {
            printf("%s:invalid vlmul: %d\n", __func__, raw_vlmul);
            return RV64_VLMUL_1;  // need to return something? todo: should set error flag or
                                  // something though.
        }
    }
}
