...
#define RV64C_QUADRANT_LOAD_STORE (0b00)



const uint32_t OP_ADDI = 0x13;
...
static uint32_t build_i_type(uint32_t opcode, uint32_t funct3, uint32_t rd, uint32_t rs1, uint32_t imm) {
    return (imm << 20) | (rs1 << 15) | (funct3 << 12) | (rd << 7) | opcode;
}


// Helper for Stores (S-Type)
static uint32_t build_s_type(uint32_t opcode, uint32_t funct3, uint32_t rs1, uint32_t rs2, uint32_t imm) {
 ...
 * @see https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_integer_register_immediate_operations
 */
static uint32_t expand_q1_addi_addiw_li(uint16_t instruction, uint8_t opcode, uint8_t funct3) {
    int32_t imm = ((instruction >> 2) & 0b11111) 
...
static uint32_t expand_q1_srli(uint16_t instruction) {
    ...
    return (OPGRP_ALU_IMMED__F3GRP_SHIFT_RIGHT__F7_SRLI << 25)
        | (shamt << 20)
        | (rd_rs1 << 15)
        | (OPGRP_ALU_IMMED__F3GRP_SHIFT_RIGHT << 12)
        | (rd_rs1 << 7)
        | OPGRP_ALU_IMMED;
}
...
static uint32_t expand_q1_srli_srai(uint16_t instruction, uint8_t opcode, uint8_t funct3, uint8_t funct7) {
    return 0;
}
        ...
        case OPGRP_CQ1: {
            switch (funct3) {
                ...
                case OPGRP_CQ1__F3GRP_ALU: {
                    ...
                    switch(funct2) {
                        case OPGRP_CQ1__F3GRP_ALU__F2_SRLI: return expand_q1_srli(instruction);
                        case OPGRP_CQ1__F3GRP_ALU__F2_SRAI: return expand_q1_srai(instruction);
                        case OPGRP_CQ1__F3GRP_ALU__F2_ANDI: return expand_q1_andi(instruction);
                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL: {
                            ...
                            switch(funct1) {
                                case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WORD: {
                                    ...
                                    switch(funct2_2) {
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WORD__F2_SUB: return expand_q1_sub(instruction);
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WORD__F2_XOR: return expand_q1_xor(instruction);
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WORD__F2_OR: return expand_q1_or(instruction);
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WORD__F2_AND: return expand_q1_and(instruction);
                                        default: return RV64_ILLEGAL_INSTRUCTION;
                                    }
                                }
                                case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WIDE: {
                                    uint8_t funct2_2 = (instruction >> 5) & 0b11;
                                    switch(funct2_2) {
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WIDE__F2_SUBW: return expand_q1_subw(instruction);
                                        case OPGRP_CQ1__F3GRP_ALU__F2GRP_LOGICAL__F1GRP_WIDE__F2_ADDW: return expand_q1_addw(instruction);
                                        default: return RV64_ILLEGAL_INSTRUCTION;
                                    }
                                }
                                default: return RV64_ILLEGAL_INSTRUCTION;
                ...
                case OPGRP_CQ1__F3_J: return expand_q1_j(instruction);
                case OPGRP_CQ1__F3_BEQZ: return expand_q1_beqz(instruction);
                case OPGRP_CQ1__F3_BNEZ: return expand_q1_bnez(instruction);
                default: return RV64_ILLEGAL_INSTRUCTION;
            }
        }
        case OPGRP_CQ2: {
