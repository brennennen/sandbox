 ...
 * @see 20.5. Single-Precision Load and Store Instructions
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_single_precision_load_and_store_instructions)
 */
static void rv64f_flw(rv64_hart_t* hart, uint8_t rd, uint8_t imm12, uint8_t rs1) {
    uint64_t base_address = hart->registers[rs1];
    uint64_t effective_address = base_address + imm12;
    memcpy(
        &hart->float32_registers[rd], &hart->shared_system->memory[effective_address], sizeof(float)
    );
}
 ...
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_single_precision_load_and_store_instructions)
 */
static void rv64f_fsw(rv64_hart_t* hart, uint8_t rs2, int16_t offset, uint8_t rs1) {
    uint64_t base_address = hart->registers[rs2];
    uint64_t effective_address = base_address + offset;
    memcpy(
        &hart->shared_system->memory[effective_address], &hart->float32_registers[rs1],
        sizeof(float)
    );
}
...
static void rv64f_fnmsub_s(
    ...
    // rd = ((-1 * rs1) * rs2)) + rs3
    hart->float32_registers[rd] = fmaf(
        (hart->float32_registers[rs1] * -1), hart->float32_registers[rs2],
 ...
 * @see 20.6. Single-Precision Floating-Point Computational Instructions
 ...
 * @see 20.7. Single-Precision Floating-Point Conversion and Move Instructions
...
static void rv64f_fsgnj_s(rv64_hart_t* hart, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    hart->float32_registers[rd] = copysignf(
        fabsf(hart->float32_registers[rs1]), hart->float32_registers[rs2]
    );
}
...
static void rv64f_fsgnjn_s(rv64_hart_t* hart, uint8_t rd, uint8_t rs1, uint8_t rs2) {
    hart->float32_registers[rd] = copysignf(
        fabsf(hart->float32_registers[rs1]), (-1 * hart->float32_registers[rs2])
    );
}
 ...
 * @see 20.8. Single-Precision Floating-Point Compare Instructions
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_single_precision_floating_point_compare_instructions)
 */
 ...
 * @see 20.9 Single-Precision Floating-Point Classify Instruction
 * (https://riscv.github.io/riscv-isa-manual/snapshot/unprivileged/#_single_precision_floating_point_classify_instruction)
 */
