 ...
 * blt tests
 * `blt rs1, rs2, <label>`
 * `blt rs1, rs2, . + <jump offset>`
 * "Branch Less Than" - If rs1 is less than rs1 (signed), add the offset to pc (branch out of
...
void rv64_emu_blt_default_setup(void) {
    memset(&g_emulator, 0, sizeof(emulator_rv64_t));
    emu_rv64_init(&g_emulator);
}

Test(emu_rv64_emulate__blt__tests, blt_take_branch, .init = rv64_emu_blt_default_setup)
{
    uint8_t input[] = {
        0x13, 0x03, 0xa0, 0x00, // li t1, 10
        0x93, 0x03, 0x40, 0x01, // li t2, 20
        0x63, 0x44, 0x73, 0x00, // blt t1, t2, next
