...
void rv64i_disassemble_register_register_default_setup(void) {
    memset(&g_emulator, 0, sizeof(emulator_rv64i_t));
    emu_rv64i_init(&g_emulator);
}

// MARK: I_RV64I_ADD_REGISTER Tests
Test(emu_rv64i_disassemble__I_RV64I_ADD__tests, add_1, .init = rv64i_disassemble_register_register_default_setup)
{
