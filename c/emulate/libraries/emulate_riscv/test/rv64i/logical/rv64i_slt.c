...
void rv64_emu_slt_default_setup(void) {
    memset(&g_emulator, 0, sizeof(emulator_rv64_t));
    emu_rv64_init(&g_emulator);
}

Test(emu_rv64_emulate__slt__tests, slt_1, .init = rv64_emu_slt_default_setup)
{
    ...
    uint8_t input[] = { 0x00, 0x73, 0x22, 0xb3 }; // slt t0, t1, t2
