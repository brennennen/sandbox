...
Test(emu_rv64_emulate__mul__tests, mul_overflow, .init = rv64_emu_mul_default_setup) {
    ...
    // mul returns the low 64 bits, so 0xFFFFFFFFFFFFFFFE (see mulh, mulhu, and mulhsu for high 64
    // bits)
    cr_assert(0xFFFFFFFFFFFFFFFE == g_emulator.harts[0].registers[RV64_REG_T0]);
}
