...
void rv64_emu_remu_default_setup(void) {
    memset(&g_emulator, 0, sizeof(emulator_rv64_t));
    emu_rv64_init(&g_emulator);
}

Test(emu_rv64_emulate__remu__tests, remu_1, .init = rv64_emu_remu_default_setup)
{
    g_emulator.registers[RV64_REG_T1] = 12;
    g_emulator.registers[RV64_REG_T2] = 5;
    uint8_t input[] = { 0xb3, 0x72, 0x73, 0x02 }; // remu t0, t1, t2
