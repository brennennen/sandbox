...
Test(
    emu_rv64_emulate__vadd_vi__tests,
    vadd_vi_sew16_vlmul1,
    .init = rv64_emu_vadd_vi_default_setup
) {
    // arrange
    rv64v_vtype_t vtype = {
        // mock: vsetvli t0, a2, e16, m1, ta, ma
        .vma = 0,
        .vta = 0,
        .selected_element_width = RV64_SEW_16,
        .vlmul = RV64_VLMUL_1
    };
    g_emulator.harts[0].csrs.vtype = rv64_csr_encode_vtype(&vtype);
...
Test(
    emu_rv64_emulate__vadd_vi__tests,
    vadd_vi_sew16_vlmul8,
    .init = rv64_emu_vadd_vi_default_setup
) {
    // arrange
    rv64v_vtype_t vtype = {
        // mock: vsetvli t0, a2, e16, m8, ta, ma
        .vma = 0,
        .vta = 0,
        .selected_element_width = RV64_SEW_16,
        .vlmul = RV64_VLMUL_8
    ...
    for (int i = 0; i < elem_per_vreg; i++) {
        ...
        cr_assert(
            i + (1 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[9].elements_16[i]
        );
        cr_assert(
            i + (2 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[10].elements_16[i]
        );
        cr_assert(
            i + (3 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[11].elements_16[i]
        );
        cr_assert(
            i + (4 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[12].elements_16[i]
        );
        cr_assert(
            i + (5 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[13].elements_16[i]
        );
        cr_assert(
            i + (6 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[14].elements_16[i]
        );
        cr_assert(
            i + (7 * elem_per_vreg) + 12 == g_emulator.harts[0].vector_registers[15].elements_16[i]
        );
    }
}
...
Test(
    emu_rv64_emulate__vadd_vi__tests,
    vadd_vi_sew32_vlmul1,
    .init = rv64_emu_vadd_vi_default_setup
) {
    // arrange
    rv64v_vtype_t vtype = {
        // mock: vsetvli t0, a2, e32, m1, ta, ma
        .vma = 0,
        .vta = 0,
        .selected_element_width = RV64_SEW_32,
        .vlmul = RV64_VLMUL_1
    };
    g_emulator.harts[0].csrs.vtype = rv64_csr_encode_vtype(&vtype);
    uint16_t elements_count = VLEN / 32;  // 4 elements (assuming 128 bit vlen)
    g_emulator.harts[0].csrs.vl =
        elements_count;  // 128 bit vlen, 32 bit elements, no grouping (vl = 128 / 32 = 4)
    for (int i = 0; i < elements_count; i++) {
...
Test(
    ...
    rv64v_vtype_t vtype = {
        ...
        .vma = 0,
        .vta = 0,
        .selected_element_width = RV64_SEW_64,
        .vlmul = RV64_VLMUL_1
    };
    g_emulator.harts[0].csrs.vtype = rv64_csr_encode_vtype(&vtype);
    uint16_t elements_count = VLEN / 64;  // 2 elements (assuming 128 bit vlen)
    g_emulator.harts[0].csrs.vl =
        elements_count;  // 128 bit vlen, 64 bit elements, no grouping (vl = 128 / 64 = 2)
    for (int i = 0; i < elements_count; i++) {
