...
result_iter_t emu_next(emulator_t* decoder) {
    decoder->current_byte = decoder->buffer[decoder->buffer_index];
    uint8_t byte1 = decoder->current_byte;
    decoder->buffer_index += 1;
    if (decoder->buffer_index >= decoder->buffer_size) {
    ...
    switch(instruction_tag) {
        // MARK: MOV
        case I_MOVE:
            result = emu_move(decoder, byte1);
            break;
        case I_MOVE_IMMEDIATE:
            result = emu_move_immediate(decoder, byte1);
            break;
        case I_MOVE_IMMEDIATE_TO_REGISTER:
            result = emu_move_immediate_to_register(decoder, byte1);
            break;
        case I_MOVE_TO_AX:
            result = emu_move_to_ax(decoder, byte1);
            break;
        case I_MOVE_AX:
            result = emu_move_ax(decoder, byte1);
            break;
        case I_MOVE_TO_SEGMENT_REGISTER:
        case I_MOVE_SEGMENT_REGISTER:
            printf("Not implemented!\n");
        ...
        // MARK: ADD
        case I_ADD:
            result = emu_add(decoder, byte1);
            break;
        case I_ADD_IMMEDIATE:
            result = emu_add_immediate(decoder, byte1);
            break;
        // ADC
        // ...
        // MARK: SUB
        case I_SUB:
            result = emu_sub(decoder, byte1);
            break;
        // ...
        // CMP
        case I_COMPARE:
            result = emu_compare(decoder, byte1);
            break;
        // ...
        // MARK: CONDITIONAL JUMPS
        case I_JUMP_ON_EQUAL:
        ...
        case I_LOOP_WHILE_NOT_EQUAL:
        case I_JUMP_ON_CX_ZERO:
            result = emu_conditional_jump(decoder, instruction_tag, byte1);
            break;
        case I_INTERRUPT_TYPE_SPECIFIED:
        case I_INTERRUPT_TYPE_3:
        case I_INTERRUPT_ON_OVERFLOW:
...
result_t emu_emulate(emulator_t* emulator, registers_t* registers) {
    for (int i = 0; i < emulator->buffer_size; i++) {
        printf("[%d] "BYTE_TO_BINARY_PATTERN"\n", i, BYTE_TO_BINARY(emulator->buffer[i]));
    }
