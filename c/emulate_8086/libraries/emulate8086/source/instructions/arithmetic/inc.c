...
emu_result_t emu_inc(emulator_t* emulator, uint8_t byte1) {
    printf("TODO: emu_inc\n");
    return ER_FAILURE;
}
...
emu_result_t decode_inc_register(
    ...
    int written = snprintf(out_buffer + *index,  out_buffer_size - *index, "inc %s\n", reg_string);
    if (written < 0) {
        return ER_FAILURE;
    }
    *index += written;
...
emu_result_t emu_inc_register(emulator_t* emulator, uint8_t byte1) {
