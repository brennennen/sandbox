...
bool gltf_viewer_init(gtlf_viewer_t* gltf_viewer) {
    log_info("Initializing engine...");
    game_engine_t* game_engine = gltf_viewer->game_engine;
    ...
    graphics_config_t graphics_config = {
        .width        = w,
        .height       = h,
        .app_name     = "GLTF/GLB Viewer",
        .present_mode = PRESENT_MODE_VSYNC, // PRESENT_MODE_VSYNC,
    };
...
int main(int argc, char* argv[]) {
    ...
    if (gltf_viewer_init(&game_engine)) {
        bool running = true;
        while (running) {
