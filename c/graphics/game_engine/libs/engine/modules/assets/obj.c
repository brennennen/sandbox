...
static bool is_vertex_equal(const vertex_t* v1, const vertex_t* v2) {
    ...
    if (fabsf(v1->pos.x - v2->pos.x) > EPSILON)
        return false;
    if (fabsf(v1->pos.y - v2->pos.y) > EPSILON)
        return false;
    if (fabsf(v1->pos.z - v2->pos.z) > EPSILON)
        return false;
    if (fabsf(v1->uv.u - v2->uv.u) > EPSILON)
        return false;
    if (fabsf(v1->uv.v - v2->uv.v) > EPSILON)
        return false;
    if (fabsf(v1->normal.x - v2->normal.x) > EPSILON)
        return false;
    if (fabsf(v1->normal.y - v2->normal.y) > EPSILON)
        return false;
    if (fabsf(v1->normal.z - v2->normal.z) > EPSILON)
        return false;
    ...
    while (fgets(line, sizeof(line), file)) {
        if (strncmp(line, "v ", 2) == 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                vertex_t v;
                v.pos.x = temp_pos[pos_idx].x;
                v.pos.y = temp_pos[pos_idx].y;
                v.pos.z = temp_pos[pos_idx].z;

                v.uv.u = temp_uvs[uv_idx * 2];
                v.uv.v = temp_uvs[uv_idx * 2 + 1];

                v.color = (vec4_t){1.0f, 1.0f, 1.0f, 1.0f};

                v.normal.x = temp_norms[norm_idx].x;
                v.normal.y = temp_norms[norm_idx].y;
                v.normal.z = temp_norms[norm_idx].z;
