...
bool cook_world(
    ...
    size_t  pak_capacity = 1024 * 1024 * 1024;
    void*   pak_memory   = malloc(pak_capacity);
    arena_t pak_arena;
    arena_init(&pak_arena, pak_memory, pak_capacity);

    world_pak_t world_pak;
    if (!process_world_source(&pak_arena, scratch_arena, world_source, base_dir_span, &world_pak)) {
        log_error("failed: process_world_source");
        return false;
    }

    int64_t pak_size = write_world_pak(&pak_arena, world_output_file);
    return true;
}
