...
// Bits definition for GPIO_IDR register (PC13)
#define GPIO_IDR_ID13_Pos           (13UL)
#define GPIO_IDR_ID13_Msk           (0x1UL << GPIO_IDR_ID13_Pos)
#define GPIO_IDR_ID13               GPIO_IDR_ID13_Msk
#define GPIO_PUPDR_PUPD13_0         (0x1UL << GPIO_PUPDR_PUPD13_Pos) /*!< 01: Pull-up */
...
int main(void) {
    ...
    RCC->IOPENR |= RCC_IOPENR_GPIOCEN; // Enable GPIO Port C

    // Configure GPIO Port A (led)
    GPIOA->MODER &= ~GPIO_MODER_MODE5; // Clear out pin 5 configurations
    GPIOA->MODER |= GPIO_MODER_MODE5_0; // Set pin 5 to output

    // Configure GPIO Port C (button)
    GPIOC->MODER &= ~GPIO_MODER_MODE13_Msk;
    GPIOC->PUPDR &= ~GPIO_PUPDR_PUPD13_Msk;
    GPIOC->PUPDR |= GPIO_PUPDR_PUPD13_0;

    // Turn on the LED
    GPIOA->ODR |= GPIO_ODR_OD5; // Set A5 high

    // Enable the LED when the button is pressed
    while (1) {
        if ((GPIOC->IDR & GPIO_IDR_ID13) == 0) {
            GPIOA->BSRR = GPIO_ODR_OD5;
        } else {
            GPIOA->BRR = GPIO_ODR_OD5;
        }
    }
}
