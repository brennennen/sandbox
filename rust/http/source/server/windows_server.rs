...
pub struct WindowsServer
{
    ...
    pub io_completion_port: winapi::um::winnt::HANDLE,
    pub streams: Vec<TcpStream>,
    pub ops: Vec<Operation>,
    pub buffers: Vec<Vec<u8>>,
    pub socket: winapi::um::winsock2::SOCKET
}
...
impl WindowsServer {
    pub fn new(port: u16,
        ...
        WindowsServer {
            ...
            event_counter: 0,
            io_completion_port: winapi::um::handleapi::INVALID_HANDLE_VALUE,
            streams: vec![],
            ops: Vec::with_capacity(5),
            buffers: Vec::with_capacity(5),
            socket: winapi::um::winsock2::INVALID_SOCKET
        }
    }
    ...
    pub fn start(&mut self) {
        self.wsa_startup();
        self.create_socket();
        WindowsServer::configure_socket_non_blocking(self.socket);
        self.create_io_completion_port();
    ...
    ///
    /// WSAStartup must be called before sockets can be used.
    ///
    fn wsa_startup(&mut self) {
        let word = winapi::shared::minwindef::MAKEWORD(2, 2);
        //let wsadata: *mut winapi::um::winsock2::WSADATA = 0 as *mut _;
        //let mut win_socket_data: winapi::um::winsock2::WSADATA = Default::default();
        let mut win_socket_data: winapi::um::winsock2::WSADATA = unsafe {
            std::mem::zeroed()
        };

        //let lpwsadata = winapi::um::winsock2::LPWSADATA;
        let wsa_startup_result = unsafe {
            winapi::um::winsock2::WSAStartup(word, &mut win_socket_data as *mut _)
        };

        if wsa_startup_result != 0 {
            panic!("Failed to startup WSA! Error: {}", std::io::Error::last_os_error());
        }
        log::debug!("WSAStartup result: {}", wsa_startup_result);
    }

    ///
    /// Creates the windows socket used by the server.
    ///
    fn create_socket(&mut self) {
        let address_family = winapi::shared::ws2def::AF_INET;
        let socket_type = winapi::um::winsock2::SOCK_STREAM;
        let protocol = winapi::shared::ws2def::IPPROTO_IP;
        self.socket = unsafe {
            winapi::um::winsock2::socket(address_family, socket_type, protocol)
        };
        if self.socket == winapi::um::winsock2::INVALID_SOCKET {
            panic!("Failed to create socket! Error: {}", std::io::Error::last_os_error());
        }
        log::debug!("Created socket on port [{}]. socket_fd: [{}]", self.port, self.socket);
    }

    fn configure_socket_non_blocking(socket: winapi::um::winsock2::SOCKET) {
        let command = winapi::um::winsock2::FIONBIO;
        //let buffer_pointer: *mut core::ffi::c_void = &mut buffer as *mut _ as *mut core::ffi::c_void;
        let argp: *mut std::os::raw::c_void = 1 as *mut _;
        let ioctlsocket_result = unsafe {
            winapi::um::winsock2::ioctlsocket(socket, command, argp as *mut _)
        };
        if ioctlsocket_result == -1 {
            panic!("Failed to get socket configurations (ioctlsocket)!");
        }
    }

    fn create_io_completion_port(&mut self) {
        self.event_counter = 0;
        let queue = unsafe {
            winapi::um::ioapiset::CreateIoCompletionPort(winapi::um::handleapi::INVALID_HANDLE_VALUE, ptr::null_mut(), 0, 0)
        };
    ...
    fn configure_buffer(&mut self) {
        ...
        self.ops = Vec::with_capacity(5);

        // and we need a place to store buffers filled by the OS
        self.buffers = Vec::with_capacity(5);
    }
}
